...
def make_sentences_tokenized(data):
	sentences = []
	sentence = [(data['word'][0], data['pos'][0], data['event_tag1'][0])]
	for i in range(1, len(data['word'])):
		if(data['index'][i] != data['index'][i-1]+1):
			sentences.append(sentence)
			sentence = [(data['word'][i], data['pos'][i], data['event_tag1'][i])]
		else:
			sentence.append(tuple((data['word'][i], data['pos'][i], data['event_tag1'][i])))
	sentences.append(sentence)
	return sentences
...
n_Eventtags = len(event_tags)
print("Number of Labels: ", n_Eventtags)
pos_tags = list(set(data['pos'].values))
print("POS Tags : ", pos_tags)
n_posTags = len(pos_tags)
print("Number of POS Tags ", n_posTags)

word2idx = {w: i+1 for i, w in enumerate(words)}
word2idx["PAD"] = 0 # Padding
idx2word = {i: w for w, i in word2idx.items()}

event2idx = {t: i+1 for i, t in enumerate(event_tags)}
event2idx["PAD"] = 0
idx2event = {i: w for w, i in event2idx.items()}

pos2idx = {t: i+1 for i, t in enumerate(pos_tags)}
pos2idx["PAD"] = 0
idx2pos = {i: w for w, i in event2idx.items()}

from keras.preprocessing.sequence import pad_sequences
X = [[word2idx[w[0]] for w in s] for s in sentences]
X = pad_sequences(maxlen = MAX_LEN, sequences = X, padding = "post", value = word2idx["PAD"])

y = [[tuple(pos2idx[w[1]], event2idx[w[2]]) for w in s] for s in sentences]
